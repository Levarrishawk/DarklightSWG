...
public:
	...
	bool canPerformSkill(CreatureObject* enhancer, CreatureObject* patient) {
		if (patient->isDead())
			return false;
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) {
		...
		uint32 heaAtt = BuffCRC::MEDICAL_ENHANCE_HEALTH;
		uint32 strAtt = BuffCRC::MEDICAL_ENHANCE_STRENGTH;
		uint32 conAtt = BuffCRC::MEDICAL_ENHANCE_CONSTITUTION;

		if (!targetCreature->hasBuff(heaAtt)) {
			attribute = BuffAttribute::HEALTH;
		} else if (!targetCreature->hasBuff(strAtt)) {
			attribute = BuffAttribute::STRENGTH;
		} else if (!targetCreature->hasBuff(conAtt)) {
			attribute = BuffAttribute::CONSTITUTION;
		} else {
			targetCreature->sendSystemMessage("You should have all 3 buffs");
		}

		if (!creature->hasBuff(heaAtt)) {
			attribute = BuffAttribute::HEALTH;
		} else if (!creature->hasBuff(strAtt)) {
			attribute = BuffAttribute::STRENGTH;
		} else if (!creature->hasBuff(conAtt)) {
			attribute = BuffAttribute::CONSTITUTION;
		} else {
			targetCreature->sendSystemMessage("You should have all 3 buffs");
		}
		...
		if (BuffAttribute::isProtection(attribute)) {  // If it's a protection enhancement, wound treatment has no effect
			buffPower = enhancer->getSkillMod("healing_ability");
			buffPower = buffPower * (1 - patient->calculateBFRatio()) * (1 - enhancer->calculateBFRatio());
		} else
			buffPower = (enhancer->getSkillMod("healing_ability") * 5);
		...
		enhancer->inflictDamage(enhancer, CreatureAttribute::ACTION, mindCost, false);
